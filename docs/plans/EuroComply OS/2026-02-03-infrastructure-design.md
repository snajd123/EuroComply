...
> **Depends On:** [Handler VM Design](./2026-02-02-compliance-handler-vm.md), [Compliance Network Design](./2026-02-02-compliance-network-design.md), [Registry Design](./2026-02-03-registry-design.md), [Platform Services Layer](./2026-02-03-platform-services-layer.md)
...
- **Each Spoke** is an independent OS instance dedicated to a single customer. It contains the full Kernel (Handler VM + Platform Services), its own databases, its own AI infrastructure, and its own Private Registry.
...
- **EuroComply OS Kernel** (Handler VM + Platform Services)
...
│  • Resolve plan (starter/professional/enterprise)               │
...
**Tier A AI is per-spoke.** Each spoke routes through its own LLM Gateway. For Starter and Professional plans, the gateway connects to a shared regional GPU pool with strict tenant isolation. Enterprise plans get dedicated GPU allocation. See [Section 10](#10-scaling--resource-profiles) for details.
...
|---|---|---|
| **Contents** | Handler VM, Platform Services, API Server, MCP Server, Worker runtime, Spoke Agent | Logic Packs, Environment Packs, Driver Packs, Intelligence Packs, GSR data |
...
STAGE 2: CANARY (Day 1-3)
  ...
  Monitor: error rates, latency, Handler VM correctness
...
|---|---|---|
| OS version (Handler VM exact build) | Compliance Lock: `handler_vm_exact` | Spoke admin or auto-update policy |
...
|---|---|---|---|
| API Server | 1 replica, 256MB, 0.25 vCPU | 2 replicas, 512MB, 0.5 vCPU | 3+ replicas, 1GB, 1 vCPU |
| MCP Server | 1 replica, 256MB, 0.25 vCPU | 2 replicas, 512MB, 0.5 vCPU | 3+ replicas, 1GB, 1 vCPU |
| Worker | 1 replica, 256MB, 0.25 vCPU | 2 replicas, 512MB, 0.5 vCPU | 3+ replicas, 1GB, 1 vCPU |
| PostgreSQL | 1 instance, 1GB, 10GB disk | 1 instance, 4GB, 50GB disk | HA cluster, 16GB, 500GB disk |
| Neo4j | 1 instance, 1GB, 10GB disk | 1 instance, 4GB, 50GB disk | HA cluster, 16GB, 500GB disk |
| Object Store | 5GB quota | 50GB quota | 1TB+ quota |
| LLM | Shared pool, best-effort | Shared pool, priority | Dedicated GPU |
...
│  • Professional:  priority queue, 50 req/min         │
...
| Component | Starter | Professional | Enterprise |
|---|---|---|---|
| K8s compute | $30/mo | $100/mo | $300+/mo |
| PostgreSQL | $20/mo | $80/mo | $400+/mo |
| Neo4j | $30/mo | $100/mo | $400+/mo |
| Object storage | $1/mo | $5/mo | $50+/mo |
| LLM (pool share) | $20/mo | $80/mo | $1,500+/mo |
| **Total** | **~$100/mo** | **~$365/mo** | **~$2,650+/mo** |
...
├── hub/
│   ├── terraform/              # Hub K8s cluster, database, networking
│   ├── helm/                   # Control plane, registry, telemetry, portals
...
│   └── operators/              # CloudNativePG, Neo4j operator
│
├── shared/
│   ├── gpu-pool/               # Regional GPU cluster + vLLM router
│   └── cert-manager/           # TLS automation
│
├── products/                   # Product manifests
│   ├── eurocomply-cosmetics.yaml
│   ├── eurocomply-biocides.yaml
│   ├── eurocomply-batteries.yaml
│   └── eurocomply-chemicals.yaml
│
└── environments/
    ├── dev/
    ├── staging/
...
The infrastructure repo contains ZERO secrets. It contains references to secrets, never the values.
...
```
Developer pushes to eurocomply-infrastructure
...
6. **No secrets in git.** Infrastructure repo contains references, never values. External Secrets Operator bridges vault to K8s. Provisioning Orchestrator writes to vault, not to CLI.
