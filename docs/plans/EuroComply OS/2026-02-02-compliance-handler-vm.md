...
> **Depends On:** [Platform Services Layer](./2026-02-03-platform-services-layer.md) (peer Kernel component)

---

## Executive Summary

The Handler Standard Library is the **instruction set** of the EuroComply Compliance Virtual Machine. These ~50 handlers are the immutable, tested, audited primitives that:
...
The Handler VM is the **compute half** of the EuroComply Kernel. It provides pure, deterministic compliance computation. Its peer is the **Platform Services Layer** ([design doc](./2026-02-03-platform-services-layer.md)), which provides stateful operations (entity CRUD, files, search, permissions).
...
**Graph Persistence:** When a `core:schedule` is active within a workflow, the `next_due` timestamp is stored on the `:IN_STATE` edge. The scheduler service polls these edges and invokes the Handler VM when evaluations are due.
...
│            (50 immutable, tested, audited primitives)           │
└─────────────────────────────────────────────────────────────────┘
```
...
Guards and effects reuse the existing AST node structure from the Handler Standard Library. No new execution model -- the VM doesn't distinguish between "evaluating a rule" and "evaluating a workflow guard."
...
When a guard or schedule references `installed_rules`, the Handler VM performs a deterministic resolution through the Identity Ladder and Compliance Lock:
                      ...
                      validation: {
                        ...
                        config: {
                          prohibited: {
                            field: 'gsr_id',
                            operator: 'in',
                            value: { list_source: { type: 'gsr_table', table: 'cosmetics_annex_ii' } }
                          }
                        }
                      },
                      require: 'all'
...
| **MCP Server** | `@modelcontextprotocol/server` | **Exposes 50 handlers as AI-callable tools** |
...
// Expose all 50 handlers as MCP tools
