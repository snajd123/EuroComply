...
> **Depends On:** [Handler Standard Library Design](./2026-02-02-compliance-handler-vm.md), [Compliance Network Design](./2026-02-02-compliance-network-design.md), [Platform Services Layer](./2026-02-03-platform-services-layer.md)

---

## Executive Summary

The Registry is the package management System Service of the EuroComply Compliance OS. It sits alongside the A2A Protocol as a peer System Service, above the Kernel (Handler VM + Platform Services) and below the Applications.
...
|                                                                       |
|  +---------+ +---------+ +---------+ +---------+ +---------+ +-----+ |
|  |REGISTRY | |  META   | |   OPS   | |   A2A   | |  DEVEL  | | UI  | |
|  |  Tools  | |  Tools  | |  Tools  | |  Tools  | |  Tools  | |Tools| |
|  +---------+ +---------+ +---------+ +---------+ +---------+ +-----+ |
|  |search   | |create_  | |get_     | |resolve_ | |scaffold | |def_ | |
|  |inspect  | | vertical| | product | | identity| |lint     | | view| |
|  |install  | |create_  | |update_  | |issue_   | |test     | |reg_ | |
|  |publish  | | rule    | | material| | claim   | |compile  | | act | |
|  |bump     | |define_  | |evaluate_| |verify_  | |         | |     | |
|  |lock     | | workflow| | compli. | | claim   | |         | |     | |
|  |diff     | |transit. | |trace_   | |request_ | |         | |     | |
|  |         | |         | | subst.  | | claim   | |         | |     | |
|  +---------+ +---------+ +---------+ +---------+ +---------+ +-----+ |
|       |            |                                  |               |
...
The `registry:compile` tool bridges the gap between raw Logic ASTs (authored by AI agents or human developers) and the validated, executable form that the Simulator runs. It acts as the "compiler" step that the Seeder performs at install time, but exposed as an MCP tool for development-time feedback.
...
| 0.2 | 2026-02-03 | Added registry:compile (Seeder bridge), devel:* toolset (scaffold, lint, test), ui:* toolset (define_view, register_action), integrated agent workflow, lint rule reference |
