...
> **Depends On:** [Handler Standard Library Design](./2026-02-02-compliance-handler-vm.md), [Platform Services Layer](./2026-02-03-platform-services-layer.md)
...
- The Handler VM is the CPU (pure computation)
...
│                                                                     │
│                        ┌─────────────┐                              │
│                        │    GSR      │  ← Shared reference data    │
│                        │ (Identity   │    (substance IDs, regs)    │
│                        │  Rosetta    │    Read-only, no trust      │
│                        │  Stone)     │    decisions here           │
│                        └─────────────┘                              │
│                              │                                      │
│              ┌───────────────┼───────────────┐                      │
│              ▼               ▼               ▼                      │
...
│     │ Handler VM  │  │ Handler VM  │  │ Handler VM  │              │
│     └──────┬──────┘  └──────┬──────┘  └──────┬──────┘              │
│            │                │                │                      │
│            └────── P2P ─────┴────── P2P ─────┘                      │
│                                                                     │
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   INTERNAL (Handler VM)             EXTERNAL (A2A Protocol)        │
│   ┌─────────────────────────┐      ┌─────────────────────────┐     │
│   │                         │      │                         │     │
...
│   │       ↓                 │      │         ↓               │     │
│   │  Handler VM             │      │  Company B's AI Agent   │     │
│   │                         │      │                         │     │
│   └─────────────────────────┘      └─────────────────────────┘     │
│                                                                     │
...
| # | Primitive | Purpose |
|---|-----------|---------|
| 1 | **Identity** | We agree what we're talking about (GSR as Rosetta Stone) |
...
**Solution:** GSR is the Rosetta Stone. All identity resolution goes through canonical substance IDs.
...
│                              │                                      │
│  LEVEL 3: COMPUTATION                                               │
│  "Handler VM evaluated: 0.0003 < 0.001 → PASS"                     │
│                              ↑                                      │
│                    computed from                                    │
│                              │                                      │
...
Every event in the system -- whether emitted by a workflow transition, a Handler VM evaluation, a temporal deadline expiry, or a manual trigger -- uses the same structure:
...
interface UniversalEvent {
  ...
  source: {
    type: 'workflow' | 'handler_vm' | 'temporal' | 'manual' | 'a2a_inbound';
    workflow_id?: string;
...
The unified event system provides hook points in the Handler VM execution lifecycle. Driver Packs and applications register for these to inject logic at well-defined moments:

| Hook Point | Event Type | Use Case |
|------------|------------|----------|
| `handler_vm.pre_evaluation` | Before any rule evaluation starts | Fetch real-time data (exchange rates, updated substance lists) |
| `handler_vm.post_evaluation` | After evaluation completes | Update dashboards, trigger notifications |
...
await mcp.call('events:subscribe', {
  subscriptions: [{
    event_type: 'handler_vm.pre_evaluation',
    filter: { source_type: 'handler_vm' },
...
**The moat:** When the protocol is open, adoption becomes unstoppable. Competitors implement your protocol. You win because your Handler VM remains the gold standard.
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                      HANDLER VM                              │   │
│  │              (The 48 immutable instructions)                 │   │
│  │                                                              │   │
│  │   core:threshold    core:for_each    core:find_path         │   │
│  │         │                │                 │                │   │
│  └─────────┼────────────────┼─────────────────┼────────────────┘   │
│            │                │                 │                    │
│            ▼                ▼                 ▼                    │
