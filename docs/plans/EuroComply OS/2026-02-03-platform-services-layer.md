...
> **Depends On:** [Handler VM Design](./2026-02-02-compliance-handler-vm.md), [Compliance Network Design](./2026-02-02-compliance-network-design.md), [Registry Design](./2026-02-03-registry-design.md)

---

## Executive Summary

The Platform Services Layer is the **stateful half of the EuroComply Kernel**. While the Handler VM provides pure, deterministic compliance computation, Platform Services provides the state management primitives that every application, pack, and agent requires: entity CRUD, file storage, search, permissions, tasks, notifications, and more.

Together, Handler VM and Platform Services form the Kernel of the Compliance OS. They are co-equal peers with a bidirectional dependency: handlers read entity data through the execution context; platform services trigger handlers for validation and computed fields.

### The Corrected Architecture

Previous design documents used a linear four-layer stack. This was incorrect -- the dependency graph between components is not strictly hierarchical. The corrected architecture uses a **ring model** inspired by real operating system design:
...
│                         KERNEL                                │
│   ┌─────────────────┐        ┌─────────────────────────┐     │
│   │   Handler VM    │◄──────►│   Platform Services     │     │
│   │   (Compute)     │        │   (State)               │     │
│   │                 │        │                         │     │
│   │   ~50 pure,     │        │   Entity CRUD, Files,   │     │
...
**Handler VM and Platform Services are peers, not stacked:**
- Handler VM needs Platform Services: handlers read entity data via `ExecutionContext`
- Platform Services needs Handler VM: computed fields, validation-on-write, workflow guards
...
- Both depend on the Kernel (Handler VM for evaluation, Platform Services for storage)
...
- Previous designs hid them inside Layer 0, creating ambiguity about what the Handler VM actually owns
...
- Handler VM = CPU instructions (ADD, MOV, CMP)
...
| **Versioning** | Tied to platform version (e.g., v2.1) | Tied to pack version (e.g., 1.0.4) |
| **Compliance Lock** | Recorded as `platform_version` | Recorded as `packs` with CID |
...
18. [The Bridge: How Handlers Reach Platform Services](#18-the-bridge-how-handlers-reach-platform-services)
19. [Kernel Service Summary](#19-kernel-service-summary)
...
Unlike Handler VM primitives (which are pure functions), Platform Services are **stateful operations with side effects**. They have a different contract:
...
### 1.2 Key Differences from Handler VM

| Property | Handler VM | Platform Services |
|----------|-----------|-------------------|
...
1. **Context reads**: Handler VM reads entity data through `ExecutionContext.entityData` -- Platform Services resolves this
2. **Computed fields**: Platform Services triggers Handler VM to recalculate computed fields after entity mutations
...
The Handler VM and Platform Services share the MCP tool namespace but use a natural semantic split:

| Namespace Owner | Prefix Style | Examples |
|----------------|-------------|----------|
| **Handler VM** | Verb/adjective-based | `core:*`, `graph:*`, `ai:*`, `meta:*`, `devel:*`, `a2a:*` |
...
```typescript
// Handler VM tool (pure computation)
...
|-----------|-------|---------|
| `core:` | Handler VM | Computation, validation, logic gates, resolution |
| `graph:` | Handler VM | Graph traversal and analysis |
| `ai:` | Handler VM + AI Runtime | Intelligence handlers + AI runtime orchestration |
| `meta:` | Handler VM | Workflow definition, rule creation, vertical setup |
| `devel:` | Handler VM | Development-time tools (scaffold, lint, test) |
  ...
  input: {
    ...
    schema: {
      fields: Array<{
        ...
        // Computed fields (evaluated by Handler VM)
        computed?: {
...
### 4.7 Relationship to Handler VM Graph Tools

Platform Services `relation:*` tools manage the graph structure (create/update/delete edges). Handler VM `graph:*` tools analyze the graph (traverse, find paths, impact analysis). They operate on the same Neo4j graph but serve different purposes:

| Tool Family | Purpose | Examples |
|-------------|---------|---------|
| `relation:*` (Platform) | Structural mutations | Create edge, delete edge, define relation type |
| `graph:*` (Handler VM) | Analytical queries | Traverse supply chain, find substitutes, impact analysis |
...
### Integration with Handler VM

When the Handler VM evaluates compliance, the `ExecutionContext` includes the entity version number. The Compliance Lock pins this version, ensuring the evaluation can be replayed against exactly the data that was evaluated.
...
The Audit Log provides a queryable, exportable record of all system actions. Unlike the Handler VM execution trace (which records compliance evaluations), the Audit Log records platform operations.
...
### Relationship to Handler VM Traces

| | Audit Log (Platform Services) | Execution Trace (Handler VM) |
|---|---|---|
...
| Tool | Purpose |
|------|---------|
| `ai:query` | Natural language question answering against platform data |
...
│  │                                                           │  │
│  │  Used by:                                                 │  │
│  │  • Handler VM AI tools (ai:classify, ai:extract,         │  │
...
│            The Handler VM executes it against real data.         │
...
| `ai:extract_from_text` | A (self-hosted) | **Yes** — SDS content, formulations | Fine-tuned 7B-13B |
...
| `ai:execute` | **Mixed** | Plan phase: B (no data). Execute phase: Handler VM (local) | Both |

### Infrastructure Cost Estimate

| Component | Setup | Monthly Cost |
|-----------|-------|-------------|
| **Tier A** (self-hosted) | 1-2x GPU (A100/H200) running quantized 7B-13B via vLLM | $1,000 - $3,000 |
| **Tier B** (cloud API) | Enterprise API for schema/rule design (low volume, user-initiated) | $500 - $2,000 |
| **Total AI infrastructure** | | **$1,500 - $5,000/mo** |
...
### Relationship to Handler VM AI Handlers

| | AI Runtime (Platform Services) | AI Handlers (Handler VM) |
|---|---|---|
...
## 18. The Bridge: How Handlers Reach Platform Services

The bridge between Handler VM and Platform Services operates through three well-defined channels:

### 18.1 ExecutionContext (VM → Services)

When the Handler VM evaluates a rule, the `ExecutionContext` provides read-only access to entity data. Platform Services resolves this:
...
The Handler VM never calls `entity:read` directly. The Kernel pre-loads the required data into the context before evaluation begins. This maintains the handler's purity -- it receives data, it doesn't fetch it.

### 18.2 Computed Fields (Services → VM)

When an entity is created or updated, Platform Services checks for computed fields and delegates to the Handler VM:

```
entity:update(product_id, { materials: [...] })
  └── Platform Services detects computed field: "total_lead_concentration"
      └── Handler VM evaluates: core:bom_sum({ source: "materials", field: "lead_ppm" })
...
meta:transition(product_id, "review" → "approved")
  └── Guard phase: Handler VM evaluates guards (pure)
...
|-----------|-----------|
| Handler VM (existing) | ~50 |
| Platform Services Tier 1 | 38 |
| Platform Services Tier 2 | 50 |
| **Total Kernel Surface** | **~138** |
...
- *[Handler VM Design](./2026-02-02-compliance-handler-vm.md) -- The compute half of the Kernel*
